...
void setup() {
  ...
  PORT->Group[PORTA].PINCFG[15].reg=(uint8_t)(PORT_PINCFG_INEN); /* PA 15 support reading back current value (even as output pin) */
  PORT->Group[PORTA].DIRSET.reg = (1 << 15); /* PA 15 set direction to ouput */
  PORT->Group[PORTA].OUTCLR.reg = (1 << 15); /* PA 15 to LOW (clear)*/
  PORT->Group[PORTA].OUTSET.reg = (1 << 15); /* PA 15 to HIGH (set)*/
